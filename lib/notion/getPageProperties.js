...
export async function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  function dealUrl(properties, NOTION_CONFIG) {
  ...
  if (properties.type === 'Post') {
    dealUrl(properties, NOTION_CONFIG)
  } else if (properties.type === 'Post_url') {
    const regex = /[0-9a-f]{32}/
    const idArr = properties.url.match(regex)
    const id = idArr?.length > 0 ? idArr[0] : null
    if (id) {
      // 获取外链页面数据
      ...
      /**
       * Post_url 修改说明:
       * 1. Post_url外链格式与Post格式对齐
       * 2. pageCover, pageIcon, title优先使用数据库数据
       */
      ...
      properties.title = !properties.title ? originPage?.title : properties.title

      properties.type = 'Post'
      properties.id = idToUuid(id)
